export class Comment {
    ...
    init = () => {
        ...
        for (const toggleCommentWriterButton of toggleCommentWriterButtons) {
            toggleCommentWriterButton.addEventListener("click", this.#toggleCreateComment);
        }
        //대댓글 생성폼 바인딩
    ...
    create = async (event) => {
        ...
        //게시글 아이디 찾기
        const postId = this.#extractPostId(commentTextArea);
        //api url 생성
        ...
        try {
            ...
            const comment = this.#parser.parseFromString(response, "text/html").querySelector(".comment");
            //이벤트리스너 바인딩
    ...
    delete = async (event) => {
        ...
        const postId = this.#extractPostId(comment);
    ...
    edit = async (event) => {
        ...
        //댓글 컴포넌트 찾고 아이디 꺼내기
        const comment = event.target.closest(".comment");
        const commentId = this.#extractCommentId(comment);
    ...
    #extractCommentId = (comment) => {
        return comment.id.substr(2);
    };

    #extractPostId = (childOfPostElement) => {
    ...
    #setCommentEventListener = (comment) => {
        ...
        comment.querySelector(".toggle-comment-writer").addEventListener("click", this.#toggleCreateComment);
