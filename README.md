...
# 1. 주요 기술 목표
- 복잡한 동적쿼리를 생성하는 코드의 복잡도 완화하기 위해 QueryDSL 적용
- 사용자 입력 검증을 위해 Bean Validation 기술 적용
- 페이지 새로고침으로 인해 의도하지 않은 Post요청을 재요청하지 않도록 PRG 패턴 적용
- 서비스 수동배포로 인한 시간낭비를 피하기 위해 자동배포기술 적용
...
- 질문글과 태그는 다대다 관계를 맺고 있습니다.  
  다대다 관계를 일대다, 다대일 관계로 만들기 위해서, 중간에 QuestionTag라는 엔티티를 추가했습니다.
...
- 질문글(Question)과 답변글(Answer)의 부모 엔티티입니다
